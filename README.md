...
- **右栏数据两节**：曲线图缩在左半边，右半边两节数据——「消耗比较」：读代码（前两轮缓存未命中与输出之和：AI 开窗头两轮集中读代码，这个数衡量新开窗口后重读代码的代价）、缓存（当前每次API请求的命中花费）、缓存失效（当前上下文全部 token 若全按 MISS 算的价钱）；「缓存」：完全失效次数、缓存时间估算与现在失效可能（占位待实现）。桌面端悬停标签各有解释，手机等触屏设备同样显示，只是没有悬停解释。
