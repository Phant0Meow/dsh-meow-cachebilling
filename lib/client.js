...
var MODEL_DATALIST_ID = "meowcb-model-catalog";
var PROVIDER_DATALIST_ID = "meowcb-provider-catalog";
function uniqueProviderFor(model, catalog) {
...
function BillingCard(props) {
  ...
  const refreshCatalog = React.useCallback(() => {
    ...
    props.scan().then(
      (rows2) => {
        if (gen === catalogGen.current) setCatalog(rows2);
      },
      () => {
      }
    );
  ...
  const base = { ...PREFILL_RATES, ...snap.base ?? {} };
  ...
  const open = (key, prefillProvider) => {
    setError(null);
    if (key === null) {
      setExpanded(null);
      setDraft(null);
  ...
  const save = async () => {
    ...
    try {
      ...
      const bad = r === false ? "宿主拒收本次写入（重启页面后重试；反复失败看浏览器控制台 [meow-cachebilling] 取证行）" : r && r.result && r.result.ok === false ? r.result.error?.message : null;
      if (bad) {
        setError(`保存被拒绝：${bad}`);
  ...
  const renderRow = (key) => {
    ...
    return isExpanded ? React.cloneElement(editor, { key }) : el(
      "div",
      {
        key,
      ...
      el(
        ...
        el(
          "button",
          {
            ...
            onClick: () => open("__new__", g.provider || void 0)
          },
          "添加"
        )
      ),
  ...
  if (catalog !== null) {
    const providersByModel = /* @__PURE__ */ new Map();
    for (const row of catalog) {
  ...
  return el(
    ...
    providerOptions.length > 0 ? el("datalist", { key: "dl-provider", id: PROVIDER_DATALIST_ID }, providerOptions) : null
  );
}
...
function applySettings(ctx, opts) {
  ...
  const scanCatalog = async () => {
    ...
    const publish = () => {
      ;
      window.__meowcbScanTrace = trace;
    };
    try {
      const candidates = [];
      try {
        const viaGet = typeof ctx.get === "function" ? ctx.get("remote.session") : void 0;
        candidates.push({ label: `ctx.get(remote.session)=${typeof viaGet?.modelCatalog}`, face: viaGet });
      } catch (e) {
        candidates.push({ label: `ctx.get threw: ${e instanceof Error ? e.message : String(e)}`, face: void 0 });
      }
      try {
        const viaProp = ctx.remote?.session;
        candidates.push({ label: `ctx.remote.session=${typeof viaProp?.modelCatalog}`, face: viaProp });
      } catch (e) {
        candidates.push({ label: `ctx.remote.session threw: ${e instanceof Error ? e.message : String(e)}`, face: void 0 });
      }
  ...
  const attach = () => {
    ...
    if (configForms === void 0 || typeof configForms.get !== "function") return;
    mounted = true;
    stopPoll();
    try {
...
var inject = ["slots", "connection", "sessions", "remote", "settingsSchema"];
function apply(ctx) {
  console.log("[meow-cachebilling] client bundle: cmp-34");
  if (typeof document !== "undefined") {
    let tag = document.querySelector(`style[data-plugin-css="${CSS_ID2}"]`);
    if (tag === null) {
      tag = document.createElement("style");
