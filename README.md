...
- **价目表可手填**：包根 `rates.yml`，provider/model 按 API 官方写法，峰谷（days × ranges 叉乘）或一口价（const）三选一，时区按计费方账单时区（IANA 名）；改完重启 `dsh web` 生效，条目写坏自动跳过并警告，绝不弄崩 DSH。
...
- **峰谷计价自动判定**：工作日高峰时段（北京时间 09:00–12:00 / 14:00–18:00）按峰价，其余时段及周六日全天按谷半价——与系统时区无关，纯事件时刻换算。账单底部小字标注当前峰谷：DeepSeek 官方路由写「梁文峰/梁文谷」，其他路由写「峰价/谷价」，一口价条目写「一口价」，估算会额外标明。
...
- 价目表在包根 `rates.yml`，手填自己的模型/API 后重启 `dsh web` 生效；文件写坏会退回内置默认表并在控制台警告。
